...
- **WHEN** a client makes a GET request to `/mobile-releases` with invalid pagination parameters (e.g., negative limit).
...
#### Scenario: Mobile release not found

- **WHEN** a client makes a GET request to `/mobile-releases/:id` with an ID that does not exist.
