...
The system SHALL provide a way to view a leaderboard aggregated by teams. It MUST dynamically aggregate scores from individual team members instead of persistently storing team-level totals.

#### Scenario: Successful team leaderboard aggregation

- **WHEN** a GET request is made to `/leaderboard/teams`
- **THEN** the system SHALL return a 200 OK status with a list of teams and their total aggregate scores, sorted in descending order by score.
